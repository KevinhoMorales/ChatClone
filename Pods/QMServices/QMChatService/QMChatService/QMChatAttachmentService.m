...
- (void)uploadAndSendAttachmentMessage:(QBChatMessage*)message toDialog:(QBChatDialog*)dialog withChatService:(QMChatService*)chatService withAttachedVideo:(NSData*)videoData completion:(QBChatCompletionBlock)completion {
    ...
    [QBRequest TUploadFile:videoData fileName:@"attachment video" contentType:@"video/mp4" isPublic:YES successBlock:^(QBResponse* response, QBCBlob* blob) {
        ...
        [self saveImageData:videoData chatAttachment:attachment error:nil];
...
- (void)uploadAndSendAttachmentMessage:(QBChatMessage*)message toDialog:(QBChatDialog*)dialog withChatService:(QMChatService*)chatService withAttachedAudio:(NSData*)audioData completion:(QBChatCompletionBlock)completion {
    
    [self changeMessageAttachmentStatus:QMMessageAttachmentStatusLoading forMessage:message];
    
    [QBRequest TUploadFile:audioData fileName:@"attachment audio" contentType:@"audio/m4a" isPublic:YES successBlock:^(QBResponse* response, QBCBlob* blob) {
        QBChatAttachment* attachment = [QBChatAttachment new];
        ...
        message.attachments = @[attachment];
        message.text = @"Attachment audio";
        
        [self saveImageData:audioData chatAttachment:attachment error:nil];
        [self.attachmentsStorage setObject:audioData forKey:attachment.ID];
...
- (void)getVideoForAttachmentMessage:(QBChatMessage *)attachmentMessage completion:(void(^)(NSError *error, NSData *data))completion {
    
    [self getDataForAttachmentMessage:attachmentMessage completion:completion];
}
- (void)getAudioForAttachmentMessage:(QBChatMessage *)attachmentMessage completion:(void(^)(NSError *error, NSData *data))completion {
