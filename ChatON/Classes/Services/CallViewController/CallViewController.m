...
- (void)viewDidLoad {
    ...
    [users insertObject:[QMApi instance].currentUser atIndex:0];
    
    for (NSNumber *uID in self.session.opponentsIDs) {
        
        if ([QMApi instance].currentUser.ID == uID.integerValue) {
            
            QBUUser *initiator = [[QMApi instance] userWithID:self.session.initiatorID.unsignedIntegerValue];
        ...
        QBUUser *user = [[QMApi instance] userWithID:uID.integerValue];
        if (!user) {
            user = [QBUUser user];
    ...
    BOOL isInitiator = ([QMApi instance].currentUser.ID == self.session.initiatorID.unsignedIntegerValue);
...
- (UIView *)videoViewWithOpponentID:(NSNumber *)opponentID {
    ...
    if ([QMApi instance].currentUser.ID == opponentID.integerValue) {//Local preview
...
- (NSIndexPath *)indexPathAtUserID:(NSNumber *)userID {
    ...
    QBUUser *user = [[QMApi instance] userWithID:userID.unsignedIntegerValue];
