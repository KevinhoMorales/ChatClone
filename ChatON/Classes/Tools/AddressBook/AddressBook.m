...
+ (void)getContactsWithEmailsWithCompletionBlock:(AddressBookResult)block
{
    ...
    [AddressBook getAllContactsFromAddressBook:^(NSArray *contacts, BOOL success, NSError *error) {
        if (success) {
            NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF.phonenumbers.@count > 0"];
